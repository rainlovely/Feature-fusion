...
from .fusion_model import convNormlayer,Repblock,CSPblock
class DINO(nn.Module):
    """ This is the Cross-Attention Detector module that performs object detection """
    def __init__(self, backbone: object, transformer: object, num_classes: object, num_queries: object,
                 convNormlayer: object,
                 CSPblock: object,
                 aux_loss: object = False, iter_update: object = False,
                 query_dim: object = 2,
                 random_refpoints_xy: object = False,
                 fix_refpoints_hw: object = -1,
                 num_feature_levels: object = 1,
                 nheads: object = 8,
                 # two stage
                 two_stage_type: object = 'no',  # ['no', 'standard']
                 two_stage_add_query_num: object = 0,
                 dec_pred_class_embed_share: object = True,
                 dec_pred_bbox_embed_share: object = True,
                 two_stage_class_embed_share: object = True,
                 two_stage_bbox_embed_share: object = True,
                 decoder_sa_type: object = 'sa',
                 num_patterns: object = 0,
                 dn_number: object = 100,
                 dn_box_noise_scale: object = 0.4,
                 dn_label_noise_ratio: object = 0.5,
                 dn_labelbook_size: object = 100,
                 #multi_scale: object = [480, 512, 544, 576, 608, 640, 640, 640, 672, 704, 736, 768, 800],
                 ) -> object:
        ...
        if num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                ...
                if _ <= 1:
                    convnorm_list.append(convNormlayer(in_channels, in_channels, kernel_size=3, stride=2, padding=1))
                    CSPlayer_list.append(CSPblock(3*in_channels,2*in_channels,num_blocks=3,expansion=0.5))
            ...
            self.convNormlayer = nn.ModuleList(convnorm_list)
            self.CSPblock = nn.ModuleList(CSPlayer_list)
        else:
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        if isinstance(samples, (list, torch.Tensor)):
            ...
            #samples.tensors = F.interpolate(samples.tensors, size=(scale, scale))
        ...
        for l, feat in enumerate(features):
            ...
            bs_tmp, ch_tmp, h_tmp, w_tmp = src.shape
            bs.append(bs_tmp)
            ch.append(ch_tmp)
            h.append(h_tmp)
            w.append(w_tmp)
            if l <= 1:
                src = self.convNormlayer[l](src)
                srcs_downsample.append(src)
        ...
        for _ in range(2):
            src_cat = torch.cat((srcs[_+1], srcs_downsample[_]), dim=1)
            srcs_f.append(self.CSPblock[_](src_cat))
            srcs_tmp.append(self.input_proj[_ + 1](srcs_f[_]))
        srcs[0] = self.input_proj[0](srcs[0])
        srcs[1] = srcs_tmp[0]
        srcs[2] = srcs_tmp[1]
